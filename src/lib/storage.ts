...
class CanvasStorage {
    ...
    private readonly DB_NAME = "infinite-canvas-db";
    private readonly DB_VERSION = 1;
    private readonly STATE_KEY = "canvas-state";
    private readonly MAX_IMAGE_SIZE = 20 * 1024 * 1024; // 20MB max per image
    private readonly MAX_VIDEO_SIZE = 60 * 1024 * 1024; // 60MB max per video
    ...
    async saveImage(dataUrl: string, id?: string, meta?: any): Promise<string> {
    ...
    async saveVideo(
        ...
        if (sizeInBytes > this.MAX_VIDEO_SIZE) {
            throw new Error(
