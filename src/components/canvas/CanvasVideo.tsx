...
interface CanvasVideoProps {
    ...
    onResizeStart?: () => void;
    onResizeEnd?: () => void;
    // Parent callback to move images and videos together during group drag
...
export const CanvasVideo: React.FC<CanvasVideoProps> = ({
    ...
    const handleDragMove = useMemo(
        () =>
            throttle((e: any) => {
                ...
                if (selectedIds.includes(video.id) && selectedIds.length > 1) {
                    if (onGroupDragMove) {
                        onGroupDragMove(video.id, node.x(), node.y());
                        return;
                    }
                    // Fallback: only update videos when parent callback is missing
                    ...
                    if (startPos) {
                        ...
                        setVideos((prev) =>
                            prev.map((vid) => {
                                if (vid.id === video.id) {
                                    return { ...vid, x: node.x(), y: node.y(), isVideo: true as const };
                                } else if (selectedIds.includes(vid.id)) {
                                    const vidStartPos = dragStartPositions.get(vid.id);
                                    if (vidStartPos) {
                                        return { ...vid, x: vidStartPos.x + deltaX, y: vidStartPos.y + deltaY, isVideo: true as const };
                                    }
                                }
                                return vid;
                            }),
        ...
        [selectedIds, video.id, dragStartPositions, setVideos, onChange, onGroupDragMove],
    );
