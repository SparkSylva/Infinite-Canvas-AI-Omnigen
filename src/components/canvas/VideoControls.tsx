...
export const VideoControls: React.FC<VideoControlsProps> = ({
  ...
  // Resolve effective duration from state or actual DOM metadata
  const getEffectiveDuration = (): number => {
    return typeof video.duration === "number" && isFinite(video.duration) && video.duration > 0
      ? video.duration
      : 0;
  };

  const isDurationReady = getEffectiveDuration() > 0;
  ...
  const handleSeekBarClick = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!seekBarRef.current || !isDurationReady) return;
  ...
  const handleSeekBarDragMove = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!isDraggingSeekBar || !seekBarRef.current || !isDurationReady) return;
  ...
  const skipForward = () => {
    const duration = getEffectiveDuration();
    const newTime = Math.min(video.currentTime + 5, duration || 0);
    onChange({ currentTime: newTime });
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <div
          ref={seekBarRef}
          className={`relative h-1.5 bg-muted rounded-full ${isDurationReady ? "cursor-pointer" : "cursor-not-allowed opacity-60"}`}
        ...
        >
          <div
            ...
            style={{ width: `${isDurationReady ? (currentTime / getEffectiveDuration()) * 100 : 0}%` }}
          />
        </div>
      ...
      <div className="flex items-center justify-center space-x-1 mt-1">
        {/* Skip backward */}
        <Button size="sm" className="h-6 w-6 p-0" onClick={skipBackward} disabled={!isDurationReady}>
        ...
        {/* Skip forward */}
        <Button size="sm" className="h-6 w-6 p-0" onClick={skipForward} disabled={!isDurationReady}>
